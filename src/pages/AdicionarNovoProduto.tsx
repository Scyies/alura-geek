...
export default function AdicionarNovoProduto() {
  ...
  async function getData() {
    const response = await axios.get(`http://localhost:5555/produtos/${productId.id}`);
  ...
  async function handleNewProduct(event: React.FormEvent<HTMLFormElement>) {
    ...
    try{
      axios.post(`http://localhost:5555/produtos/novo-produto`, {
  ...
  async function handleEditProduct(event: React.FormEvent<HTMLFormElement>) {
    ...
    try{
      ...
      axios.put(`http://localhost:5555/produtos/edit/${productId.id}`, {
        nome: String(input.nome),
        descricao: String(input.descricao),
        imagem: String(input.imagem),
        preco: String(input.preco),
  ...
  return (
    <>
      <main className="bg-black/10 p-4 md:p-8 lg:py-12">
        <form onSubmit={!productId ? handleNewProduct : handleEditProduct} className="flex flex-col gap-4 lg:max-w-xl lg:mx-auto">
          <h1 className="text-gray font-bold text-xl lg:text-3xl">Adicionar novo produto</h1>
          <InputWLabel label="URL da imagem" defaultValue={data && data?.imagem} name="imagem" />
          <div className="flex flex-col bg-white rounded-md shadow-md">
            <label htmlFor="" className="text-sm text-gray/50 px-3 pt-2">Categoria</label>
            <select name="row" value={selector} onChange={(e) => setSelector(e.target.value)} id="" className="py-1 px-2 rounded-md bg-white outline-none">
          ...
          <InputWLabel label="Nome do produto" defaultValue={data && data?.nome} name="nome" />
          <InputWLabel label="Preço do produto" defaultValue={data && data?.preco} name="preco" />
          <textarea
            name="descricao"
            id=""
            cols={40}
          ...
          ></textarea>
          <Button variant="secondary" type="submit">Adicionar produto</Button>
        </form>
      </main>
      <Info />
      <Footer />
